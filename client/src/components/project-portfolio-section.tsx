...
type ThumbnailFallbackStage = (typeof thumbnailFallbackStages)[number];
...
export function ProjectPortfolioSection({ projects = [] }: ProjectPortfolioProps) {
  ...
  const [thumbnailFallbackStage, setThumbnailFallbackStage] = useState<Record<string, number>>({});
  ...
  const getYouTubeThumbnail = (
    url: string,
    quality: Exclude<ThumbnailFallbackStage, "projectThumbnail"> = "maxresdefault"
  ) => {
    const videoId = getYouTubeVideoId(url);
    return videoId ? `https://img.youtube.com/vi/${videoId}/${quality}.jpg` : null;
  };

  const resolveThumbnailUrl = (project: ProjectVideo) => {
    const videoId = getYouTubeVideoId(project.youtubeUrl);
    const stageIndex = thumbnailFallbackStage[project.id] ?? 0;
    const stage = videoId ? thumbnailFallbackStages[stageIndex] : "projectThumbnail";

    if (!stage) {
      return null;
    }

    if (stage === "projectThumbnail") {
      return project.thumbnailUrl ?? null;
    }

    return getYouTubeThumbnail(project.youtubeUrl, stage);
  };

  const handleThumbnailError = (projectId: string) => {
    setThumbnailFallbackStage((prev) => {
      const currentStage = prev[projectId] ?? 0;
      return {
        ...prev,
        [projectId]: currentStage + 1,
      };
    });
  };
